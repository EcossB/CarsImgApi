...
Log.Logger = new LoggerConfiguration()
    ...
    .WriteTo.File("logs/errores-.txt",
        rollingInterval: RollingInterval.Day, // Crea un archivo nuevo cada d�a autom�ticamente
...
{
    Log.Information(@"-------------------------------------------------------------
                      Iniciando la aplicaci�n CarsImgApi...
                      -------------------------------------------------------------");
    ...
    app.UseExceptionHandler(errorApp =>
    {
        errorApp.Run(async context => {
            ...
            if (contextFeature != null)
            {
                ...
                // 4. AQU� USAMOS SERILOG.
                // Log.Error guardar� la fecha, el mensaje, la ruta y el StackTrace completo.
                Log.Error(excepcion, "Fallo cr�tico no manejado al intentar acceder a {Ruta}", ruta);
                ...
                {
                    Mensaje = "Ocurri� un error inesperado. Nuestro equipo t�cnico ya fue notificado.",
                    IdError = ticketId
                });
            }
...
{
    Log.Fatal(ex, "La aplicaci�n fall� al arrancar de forma catastr�fica.");
}
finally
{
    // 6. Asegura que los �ltimos logs se escriban antes de que se apague el programa
    Log.CloseAndFlush();
}
